...
def start(message):
    text_message = ('''Привет! Я помогу тебе составить рацион на неделю, 
...
def choose_param(message, option=None):
    ...
    calories = InlineKeyboardButton('калорийность',
                                    callback_data='calories')
    time = InlineKeyboardButton('время готовки', callback_data='time')
    products = InlineKeyboardButton('количество продуктов',
                                    callback_data='products')
    spicy = InlineKeyboardButton('острота', callback_data='spicy')
    complexity = InlineKeyboardButton('сложноть',
                                      callback_data='complexity')
    blacklist = InlineKeyboardButton('черный список',
                                     callback_data='blacklist')
    generate = InlineKeyboardButton('составить меню',
    ...
    if option:
        ...
        bot.edit_message_media(media=media, chat_id=message.chat.id,
                               message_id=message.message_id,
                               reply_markup=markup)
    else:
        bot.send_photo(message.chat.id, photo=photo, caption=txt,
                       parse_mode='html', reply_markup=markup)
...
def modify_calories(call: types.CallbackQuery):
    bot.edit_message_text(chat_id=call.message.chat.id,
                          message_id=call.message.id, text='Введи калорийность в ккал')
...
def read_products(filename):
    file = open(filename)
    data = json.load(file)
    products = [product[0] for product in
                data['list_of_products'].items()]
    return products
...
def send_product_list(message, page=0):
    ...
    bot.send_message(message.chat.id, "Выберите продукты, чтобы добавить в
    черный
    список: ", reply_markup=markup)
...
def create_product_markup(page):
    ...
    for product in products[start:end]:
        ...
        markup.add(InlineKeyboardButton(product + emoji,
    ...
    markup.add(InlineKeyboardButton("Добавить продукты в черный список",
...
@bot.callback_query_handler(func=lambda call:
call.data.startswith("confirm"))
def confirm_handler(call: CallbackQuery):
    selected_products = [product for product, status in
                         product_status.items() if status]
...
{recipe['pfc'][0]} / {recipe['pfc'][1]} / {recipe['pfc'][2]}\n
"
...
def create_navigation_buttons(current_day, mess_id):
    days = ['Список покупок', 'Понедельник', 'Вторник', 'Среда',
            'Четверг', 'Пятница', 'Суббота', 'Воскресенье']
    markup = InlineKeyboardMarkup()

    if current_day == -1:
        next_day = InlineKeyboardButton(days[1],
                                        callback_data=f"next_{current_day}_{mess_id}")
        markup.add(next_day)
        blacklist = InlineKeyboardButton("Добавить в черный список",
                                         callback_data=f"list_{current_day}_{mess_id}")
        markup.add(blacklist)
    elif current_day == 6:
        prev_day = InlineKeyboardButton(days[current_day],
                                        callback_data=f"prev_{current_day}_{mess_id}")
        markup.add(prev_day)
        change_breakfast = InlineKeyboardButton("Заменить завтрак",

                                                callback_data=f"change_breakfast_{current_day}_{mess_id}")
        change_lunch = InlineKeyboardButton("Заменить обед",
                                            callback_data=f"change_lunch_{current_day}_{mess_id}")
        change_dinner = InlineKeyboardButton("Заменить ужин",
                                             callback_data=f"change_dinner_{current_day}_{mess_id}")
        change_day = InlineKeyboardButton("Заменить день",
                                          callback_data=f"change_day_{current_day}_{mess_id}")
        ...
        prev_day = InlineKeyboardButton(days[current_day],
                                        callback_data=f"prev_{current_day}_{mess_id}")
        next_day = InlineKeyboardButton(days[current_day + 2],
                                        callback_data=f"next_{current_day}_{mess_id}")
    ...
    main_menu = InlineKeyboardButton("Главное меню",
                                     callback_data=f"main_menu_{current_day}_{mess_id}")
...
def get_menu(call: types.CallbackQuery):
    try:
        ...
        user_response = requests.post(f"{FASTAPI_URL}/user",
                                      params=user_payload)
        ...
        bot.edit_message_media(media=media, chat_id=call.message.chat.id,
...
@bot.callback_query_handler(func=lambda call:
call.data.startswith(('prev_', 'next_', 'list_', 'main_menu')))
def navigate_menu(call: types.CallbackQuery):
    ...
    try:
        ...
        user_response = requests.get(f"{FASTAPI_URL}/user",
                                     params={"chat_id": chat_id, "mess_id": mess_id})
        user_response.raise_for_status()
